...
- 2026-05-26: Pulse stats panel landed on `/leagues/[slug]/leaderboard`. Server page parallel-loads `loadLeaguePulse(leagueId, userId)` + `loadTournamentPulse(leagueId)` from `lib/stats/pulse.ts` and mounts `<PulseTabs>` below `LeaderboardLive`. League mode shows 4 tiles (leader's gap, hottest current streak, viewer's bold picks, viewer's perfect MDs) + 3 highlights (most-contested match, group consensus, wildcard pick). Tournament mode shows 4 tiles (avg goals / finished match, upsets at ≥5 rank delta, total goals, red cards from `player_card_log`) + 3 highlights (most-picked champion, top golden boot, Cinderella vote). All highlight cohorts are league-scoped — issue #37's `loadTournamentPulse()` signature was narrowed from "tournament-wide" to "league-scoped highlights + real match-fact tiles" to stay inside RLS without needing `supabaseService()`. The tournament `unstable_cache` AC was skipped for v1 (queries are small and Pulse is below the fold); see `components/stats/CLAUDE.md` for the rewire path. The `match_pick_distribution` view AC was also skipped — distribution is aggregated in TS per the issue's "no SQL window needed for league-sized cohorts" hint. Closes #37.
