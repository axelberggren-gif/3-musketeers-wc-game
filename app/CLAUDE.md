...
- 2026-05-28: Landing CTA in `app/page.tsx` renamed from "▶ Start the album" to "▶ Start predicting"; `app/layout.tsx` meta description dropped the "Sticker album for fixtures." sentence in favour of a plain prediction-league summary. Issue #44 resolved as option 3 ("drop the album metaphor from UX copy") — the sticker visual identity stays, only labels change. `DESIGN_MISALIGNMENTS.md` §5 (Album progress strip) removed as a target feature; §14 ("Album" naming) rewritten as a decision record. Closes #44.
