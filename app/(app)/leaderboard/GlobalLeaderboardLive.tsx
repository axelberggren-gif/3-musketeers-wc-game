...
interface Props {
  initialRows: GlobalStandingsRow[];
  currentUserId: string | null;
}

export function GlobalLeaderboardLive({ initialRows, currentUserId }: Props) {
  const [rows, setRows] = useState(initialRows);

  useEffect(() => {
    const supabase = supabaseBrowser();
    // Live aggregation accessor (migration 0031) — sums every user's global
    // (league_id IS NULL) awards across all leagues.
    const refetch = async () => {
      const { data } = await supabase
        .rpc("get_global_standings")
        .order("total_points", { ascending: false });
      if (data) setRows(data as GlobalStandingsRow[]);
    };
  ...
  if (rows.length === 0) {
    return (
      <div className="card text-sm text-ink-soft">
        No points awarded yet. The board fills up as matches finish.
      </div>
    );
  }
