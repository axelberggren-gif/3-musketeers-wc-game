...
- `BracketBuilder.tsx` — **"The Wall Chart"**: a symmetric tournament poster (left draw
  ...
  - **build** (`!locked`) — editable. Each slot is a `MatchCell` with two stacked
    ...
    recursive team codes). **R32 entry cells fill from the imported real fixture
    only, per side**: each side shows its real team as soon as football-data sets it
    on the fixture, and an unresolved side shows a neutral `To be decided` placeholder.
    `slotMatches[slot]` records a slot as soon as **either** side has a team (was: only
    when both did) and is read per-side.
