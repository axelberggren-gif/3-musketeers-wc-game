...
- `points-sync.test.ts` — the SQL↔TS points-sync guard: parses
  `supabase/migrations/*.sql` in lexical apply order (last `create or replace`
  wins, `drop function` removes) to derive the **live** `points_*()` set, then
  asserts it against `POINTS` in BOTH directions (every mapped constant returns
  its TS twin's value; no unmapped or missing functions) plus
  `points_bracket_slot()`'s CASE arms against `POINTS.bracket`. A point-value
  change is a three-legged edit: new migration + `rules.ts` twin + the
  `SQL_CONSTANT_MAPPING` entry in this test.
...
- Lock logic is gated by **two timestamps on the `tournament` row**:
...
- 2026-06-09: New `points-sync.test.ts` closes the SQL side of the points-sync invariant. `rules.test.ts` pins every `POINTS` leaf, but CI never executes SQL — so a migration that redefined (or dropped) a `points_*()` constant merged green while the DB and `rules.ts` silently diverged; only TS-side drift was caught. The test parses `supabase/migrations/*.sql` in lexical apply order (comments stripped; last `create or replace` wins, `drop function` removes — exactly the state Postgres ends up with) and asserts the live function set against an explicit `SQL_CONSTANT_MAPPING` table in **both** directions (mapped constants equal their `POINTS` twins; nothing unmapped, nothing missing — `points_dark_horse`/`points_group_winner` asserted as staying dropped), plus `points_bracket_slot()`'s CASE arms (incl. the `else 0` fall-through) against `POINTS.bracket`. SQL-side drift now fails `npm test`/CI. No point values touched.
