...
- 2026-06-08: `FootballDataClient.req()` hardened against transient upstream/network failures (Sentry `JAVASCRIPT-NEXTJS-7`: bare `TypeError: fetch failed` at `POST /api/cron/sync-fixtures`, 6 events). Adds a 10s per-request timeout via `AbortSignal.timeout()` and a retry loop (3 attempts; 500ms / 1000ms exponential backoff) that retries only Node undici's transient errors — `TypeError: fetch failed` (DNS/TCP-reset/TLS), `TimeoutError`, `AbortError` — and 5xx responses. 4xx still fails fast on the first attempt (those are our bug, not a blip). The non-ok body is preserved in the thrown error message. Worst-case wall time ~32s. No public-surface change — `teams()`/`matches()`/`match()`/`scorers()` callers are unchanged. Pure-function mapper tests in `client.test.ts` are unaffected.
