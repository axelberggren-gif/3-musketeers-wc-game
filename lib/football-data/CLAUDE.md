...
- 2026-06-09: `seedTeams()` (after the team upsert) and `syncFixtures()` (alongside the `backfill_team_group_letters` call) now invoke the `backfill_team_fifa_rankings` RPC (migration `0029_backfill_fifa_rankings.sql`), so `teams.fifa_ranking` self-heals on every sync — nothing in TS ever wrote it, and on a fresh deploy 0005's one-shot seed ran before any team rows existed, leaving the column NULL and silently zeroing dark-horse scoring. Idempotent, zero football-data API cost, best-effort like the other post-upsert RPCs. Those best-effort `catch {}` blocks (six across `sync.ts`) now carry intent comments ("Best-effort: idempotent RPC, the next cron run retries") so ESLint's newly-enabled `no-empty` passes and the swallow is documented as deliberate.
