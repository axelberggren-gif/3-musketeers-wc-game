...
- 2026-06-05: `0016_reconcile_group_stage_props.sql` — brings the two group-stage props under 0014's reconcile model, closing the documented follow-up. `score_group_winner(group)` and `score_first_eliminated()` were one-shot: they early-returned on the `group_settlements` / `first_elimination` write-once latch, so a group result corrected post-settlement (admin override or football-data revision) could never move the 5-pt group-winner or 10-pt first-eliminated award. Both now drop the early-return, re-derive the latch from the current result via `on conflict (...) do update ... where <col> is distinct from excluded.<col>` (no-op when unchanged), DELETE the awards they own whose prediction no longer matches the current winner / eliminated team (stale-only predicate), then re-INSERT `on conflict do nothing`; if the result is no longer decided (a FINISHED match reverted) they delete every owned award and clear the latch. `settle_group_stage_props()` drops its `not exists (group_settlements ...)` filter and calls `score_group_winner(g)` for every group every run (safe — the scorers are idempotent on unchanged results). Point values unchanged (`points_group_winner()` = 5, `points_first_eliminated()` = 10), so points-sync with `rules.ts` holds. No schema change → no `npm run db:types`. Dropping `first_elimination`'s write-once guard also lets a revised approximation self-correct (partially helps #81; the selection algorithm is unchanged). pgTAP coverage deferred to #70. Closes #82.
